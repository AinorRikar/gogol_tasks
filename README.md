...
- `src/shared` — общие типы и API-клиент.
- `src/entities` — сущности интерфейса (карточка проекта).
- `src/features` — функциональные блоки (авторизация, тема, создание проекта).
- `src/widgets` — компоновка блоков (список проектов).
- `src/pages` — страницы и бизнес-сценарии.
- `src/layouts` — общий layout приложения.

Бэкенд расположен в `server/api` (эндпоинты) и `server/utils` (утилиты auth/prisma/project).
...
- `Project` — проект (status, visibility, hidden, archivedAt).
...
- `nuxt.config.ts`
  - SSR включен;
  - auto-import компонентов по FSD-папкам;
